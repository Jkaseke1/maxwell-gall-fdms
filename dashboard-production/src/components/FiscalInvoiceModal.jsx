  ...
  return createPortal(<div id="invoice-print-root" className="fixed inset-0 z-50 bg-black/70 p-4 flex justify-center items-start overflow-auto">
    <div id="invoice-print-dialog" className="w-full max-w-5xl bg-white rounded-lg my-4">
      ...
      <article id="printable-invoice" className="p-8 text-slate-900 text-xs">
        ...
        <div className="border border-amber-600 text-amber-900 p-3 mb-5 font-bold">ZIMRA PRODUCTION FISCAL DOCUMENT<br/>{preview?'NOT SUBMITTED TO ZIMRA':accepted?'Accepted without validation errors':'REQUIRES REVIEW'}{copy?' — COPY':''}{accepted&&isPlaceholderAddress(display?.address)&&<><br/><span className="font-normal">ZIMRA branch address is still pending correction by ZIMRA; the address below is the live registered value.</span></>}</div>
