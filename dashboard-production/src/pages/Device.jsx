...
export default function Device() {
  ...
  const cards = [['Connection', live ? 'Online' : 'Offline'], ['Heartbeat', live?.maintenance?.error ? 'Attention' : 'Online'], ['Device ID', live?.deviceId ?? '—'], ['Serial number', live?.serial ?? '—'], ['Environment', live?.environment ?? 'PRODUCTION'], ['Fiscal day', live?.fiscalDayNo ?? '—'], ['Fiscal-day status', live?.status?.fiscalDayStatus ?? '—'], ['Next global receipt', live?.nextGlobalNo ?? '—'], ['Day counter', live?.nextCounter ?? '—']];
  const rows = [['Taxpayer', live?.config?.taxPayerName], ['TIN', live?.config?.taxPayerTIN], ['VAT number', live?.config?.vatNumber], ['Branch address', address], ['Operating mode', live?.config?.deviceOperatingMode], ['Certificate valid until', live?.certificateValidTill], ['QR verification host', live?.config?.qrUrl], ['Maintenance heartbeat', live?.maintenance?.error || 'Healthy'], ['Counter alignment', live?.ready ? 'Ready' : (live?.message || 'Needs attention')]];
