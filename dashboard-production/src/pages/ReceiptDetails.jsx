...
export default function ReceiptDetails() {
  ...
  useEffect(() => {
    ...
    fetch('/api/receipts').then(async r => {
      ...
      if (active) {
        setInvoice(found);
        fetch('/api/event', { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-Maxwell-Production': '46158' }, body: JSON.stringify({ type: 'invoice.viewed', invoiceNumber: found.invoice_number }) })
  ...
  const officialUrl = invoice?.zimra_verification_url?.startsWith('https://fdms.zimra.co.zw/') ? invoice.zimra_verification_url : null;
  ...
  return <main className="min-h-screen bg-white px-6 py-10 text-slate-900">
    <div className="max-w-lg mx-auto space-y-7">
      ...
      <p className="text-center text-sm text-amber-800 bg-amber-50 border border-amber-200 rounded-lg p-3">Maxwell Glass · Production receipt details<br />Local archive view — official validation is provided by ZIMRA.</p>
      {error ? <p role="alert">{error}</p> : !invoice ? <p>Loading saved receipt…</p> : <>
        ...
        <p className="text-sm text-slate-600">The company address above comes from Maxwell Glass settings. ZIMRA’s production branch address is managed separately and may still show its test placeholder.</p>
