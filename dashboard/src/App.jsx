...
function Sidebar() {
  ...
  return <aside className="erp-sidebar">
    ...
    <div className="erp-sidebar-foot"><ShieldCheck size={16} /><span>Sandbox isolated<br />Device 38293 only</span></div>
  </aside>;
}

function Header() {
  const [live, setLive] = React.useState(null);
  const load = React.useCallback(async () => { try { const response = await fetch('/sandbox-api/status'); if (!response.ok) throw Error(); setLive(await response.json()); } catch { setLive(null); } }, []);
