...
const isPlaceholderAddress = a => /001,?\s*Test Street,?\s*Test City,?\s*Test Province/i.test(address(a));
...
export default function FiscalInvoiceModal({invoice, onClose, onPrint}) {
  ...
  const title=preview ? 'TEST PREVIEW' : invoice.receipt_type==='CreditNote' ? 'CREDIT NOTE' : invoice.receipt_type==='DebitNote' ? 'DEBIT NOTE' : 'FISCAL TAX INVOICE';
  const code=invoice.zimra_verification_url?.match(/([A-Fa-f0-9]{16})$/)?.[1]?.toUpperCase().match(/.{4}/g)?.join('-');
  const handlePrint=async()=>{
    try {
      setError('');
      if (invoice.receipt_id) {
        const r=await fetch('/sandbox-api/print',{method:'POST',headers:{'Content-Type':'application/json','X-Maxwell-Sandbox':'38293'},body:JSON.stringify({invoiceNumber:invoice.invoice_number})});
  ...
  return createPortal(<div id="invoice-print-root" className="fixed inset-0 z-50 bg-black/70 p-4 flex justify-center items-start overflow-auto">
    <div id="invoice-print-dialog" className="w-full max-w-5xl bg-white rounded-lg my-4">
      ...
      <article id="printable-invoice" className="p-8 text-slate-900 text-xs">
        ...
        <div className="border border-amber-600 text-amber-900 p-3 mb-5 font-bold">ZIMRA SANDBOX TEST — NOT A PRODUCTION TAX INVOICE<br/>{preview?'NOT SUBMITTED TO ZIMRA':accepted?'Accepted without validation errors':'REQUIRES REVIEW'}{copy?' — COPY':''}{accepted&&isPlaceholderAddress(display?.address)&&<><br/><span className="font-normal">ZIMRA branch address is still pending correction by ZIMRA; the address below is the live registered value.</span></>}</div>
        ...
        <p className="mt-4">{invoice.notes}</p>
