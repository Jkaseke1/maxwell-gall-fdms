...
export default function Audit() {
  ...
  const backup=async()=>{try{const r=await fetch('/sandbox-api/backup',{method:'POST',headers:{'Content-Type':'application/json','X-Maxwell-Sandbox':'38293'},body:'{}'});const x=await r.json();if(!r.ok)throw Error(x.error);setBackupStatus('Verified local backup: '+x.destination+'. Device keys remain separately in certs/test-38293.');}catch(e){setBackupStatus(e.message);}};
  const [events, setEvents] = useState([]), [error, setError] = useState(''), [search, setSearch] = useState('');
  const [invoices, setInvoices] = useState([]);
  const refresh = async () => {
    try { const [r, records] = await Promise.all([fetch('/sandbox-api/audit'), fetch('/sandbox-api/archive')]); if (!r.ok || !records.ok) throw Error('Audit archive unavailable'); setEvents(await r.json()); setInvoices(await records.json()); setError(''); }
  ...
  return <section className="space-y-4 max-w-6xl mx-auto">
    ...
    <p className="text-sm">Sandbox device 38293 · Latest 500 events. Export includes all archived invoices, signed payloads, ZIMRA responses, draft history and numbering reservations.</p>
    <div className="flex gap-3 items-center flex-wrap">
      ...
      <a href="/sandbox-api/export" className="bg-emerald-700 text-white rounded p-2">Download full audit archive (JSON)</a>
    </div>
    {error && <p role="alert" className="text-red-700">{error}</p>}
    {backupStatus&&<p role="status" className="text-sm">{backupStatus}</p>}
    <p className="text-xs text-gray-600">Stored locally in data/maxwell-glass/sandbox-38293. Print events record a request, not proof that paper or a PDF was produced. Staff identity is not recorded until user logins are added.</p>
