...
import { TEST_TAXES, calculateInvoice, buildInvoicePreview } from '../utils/invoicePreview.mjs';
...
export default function CreateInvoice({ mode = 'invoice' }) {
  ...
  const api = async (route, body) => {
    const response = await fetch('/sandbox-api/' + route, body === undefined ? {} : { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-Maxwell-Sandbox': '38293' }, body: JSON.stringify(body) });
    const payload = await response.json(); if (!response.ok) throw Error(payload.error || 'Request failed.'); return payload;
  };
  const refresh = async () => {
    try { const [status, receipts] = await Promise.all([api('status'), api('receipts')]); setLive(status); setHistory(receipts); } catch (e) { setError('Sandbox API: ' + e.message); }
  };
