// Tax choices returned by GetConfig for test device 38293, 2026-09-05.
...
export function calculateInvoice(items, taxId, receiptType = 'FiscalInvoice') {
  const tax = TEST_TAXES.find(t => t.id === Number(taxId));
  ...
  const lines = items.map((item, index) => {
    const tax = TEST_TAXES.find(t => t.id === Number(item.taxId ?? taxId)); if(!tax) throw Error('Invalid line tax');
  ...
  const taxAmount = round([...groups].reduce((s,[id,net])=>s + Math.sign(net)*round(Math.abs(net)*(TEST_TAXES.find(t=>t.id===id).rate||0)/100),0));
...
export function buildInvoicePreview({ items, taxId, invoiceNumber, currency, customer, amountPaid, receiptType = 'FiscalInvoice', paymentMethod = 'Cash', notes = '' }) {
  ...
  return {
    ...
    status: 'LOCAL_PREVIEW', environment: 'TEST', device_id: '38293',
    device_serial: 'TEST-2000945150-B670',
