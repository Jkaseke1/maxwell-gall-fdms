...
const { output } = require('../../scripts/maxwellProductionTransport');
...
try { lock = fs.openSync(lockPath, 'wx'); } catch (error) {
  ...
  if (!Number.isInteger(oldPid) || oldPid <= 0) throw Error('Invalid production API lock; inspect before recovery.');
  let stale = false;
  try { process.kill(oldPid, 0); } catch (e) { if (e.code === 'ESRCH') stale = true; else throw e; }
  if (!stale) throw Error('Production API is already running or its lock process is still active.');
...
const allowedOrigins = new Set(['http://127.0.0.1:46157', 'http://localhost:46157']);
const server = http.createServer(async (req, res) => {
  ...
  res.setHeader('Access-Control-Allow-Headers', 'Content-Type, X-Maxwell-Production');
  res.setHeader('Access-Control-Allow-Methods', 'GET, POST, OPTIONS');
  if (req.method === 'OPTIONS') { res.writeHead(204); return res.end(); }
  try {
    let result;
    if (req.method === 'GET' && req.url === '/api/status') result = await service.live();
    else if (req.method === 'GET' && req.url === '/api/receipts') result = service.state.receipts.map(r => {
    ...
    else if (req.method === 'GET' && req.url === '/api/audit') result = service.archive.events.slice(-500).reverse();
    else if (req.method === 'GET' && req.url === '/api/archive') result = service.archive.receipts();
    else if (req.method === 'GET' && req.url === '/api/reports') result = require('./reports').reports(service.state, service.latestConfig);
    else if (req.method === 'GET' && /^\/api\/report-print\?day=\d+$/.test(req.url)) {
      ...
      res.setHeader('Content-Disposition', 'attachment; filename="maxwell-glass-production-audit.json"');
      result = service.archive.export();
    }
    else if (req.method === 'POST' && ['/api/open-day', '/api/close-day', '/api/receipts', '/api/number', '/api/draft', '/api/event', '/api/renew-certificate', '/api/print', '/api/backup'].includes(req.url)) {
      if (req.headers['x-maxwell-production'] !== '46158' || !req.headers['content-type']?.startsWith('application/json')) throw Error('Maxwell production request headers required.');
      let body = '';
...
server.listen(46158, '127.0.0.1', () => console.log('MAXWELL GLASS production API: http://127.0.0.1:46158 (device 46158 only)'));
