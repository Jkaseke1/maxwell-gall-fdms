...
function identity(config) {
  if (config.taxPayerTIN !== '2000945150' || config.vatNumber !== '220438802' || config.deviceSerialNo !== 'ZIMRAVD-1737') throw Error('Maxwell Glass identity mismatch.');
...
function prepare(input, config, counters, key, now = localTime()) {
  ...
  const receipt = { receiptType, receiptCurrency: input.currency, invoiceNo,
    ...
    receiptNotes: 'MAXWELL GLASS PRODUCTION FISCAL DOCUMENT' + (input.notes ? ': ' + input.notes.trim() : '') };
  const hashInput = buildReceiptHashInput({ ...receipt, deviceID: 46158 }, counters.previousHash);
  ...
  return { receipt, subtotal, taxAmount, total, paid, seller: { name: config.taxPayerName, tin: config.taxPayerTIN, vatNo: config.vatNumber, branch: config.deviceBranchName, address: config.deviceBranchAddress, contacts: config.deviceBranchContacts, deviceId: 46158, serialNo: config.deviceSerialNo }, configSnapshot: config };
}
function printed(input, prepared, response, fiscalDayNo, qrBase) {
  ...
  if (!qrBase || qrBase.includes('fdmstest')) throw Error('Unexpected production QR host.');
  return { invoice_number: r.invoiceNo, invoice_date: r.receiptDate, currency: r.receiptCurrency,
    ...
    balance_due: round(prepared.total - prepared.paid), status: valid ? 'FISCALIZED' : 'VALIDATION_ERRORS', environment: 'PRODUCTION',
    device_id: '46158', device_serial: 'ZIMRAVD-1737', receipt_number: r.receiptGlobalNo,
    ...
    validation_errors: errors, zimra_verification_url: generateQRData(qrBase, 46158, r.receiptDate, r.receiptGlobalNo, r.receiptDeviceSignature.signature) };
}
function closePayload(state, key) {
  ...
  const hashInput = buildFiscalDayHashInput({ deviceID: 46158, fiscalDayNo: state.fiscalDayNo, fiscalDayDate: state.openedAt }, counters);
