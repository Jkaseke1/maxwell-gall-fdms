...
class ProductionService {
  constructor({ request = transport.request, directory = transport.output, key = fs.readFileSync(transport.keyPath), verifyReceipt = certificates.verifyReceipt } = {}) {
    ...
    this.archive = new Archive(directory === transport.output ? path.resolve(__dirname, '../../data/maxwell-glass/production-46158') : path.join(directory, 'archive'));
    ...
    this.state = fs.existsSync(this.file) ? JSON.parse(fs.readFileSync(this.file)) : { deviceId: 46158, lastGlobalNo: 0, counter: 0, receipts: [] };
    if (this.state.deviceId !== 46158) throw Error('Saved state belongs to another device.');
    this.busy = false;
  ...
  async exclusive(fn) {
    if (this.busy) throw Error('Another production operation is in progress.');
    this.busy = true; try { return await fn(); } finally { this.busy = false; }
  }
  async live() {
    const config = await this.request('/Device/v1/46158/GetConfig', undefined, true);
    identity(config);
    const status = await this.request('/Device/v1/46158/GetStatus', undefined, true);
    ...
    return { config, status, deviceId: 46158, serial: config.deviceSerialNo, environment: 'PRODUCTION',
      ...
      message: !aligned ? 'Server and local receipt history differ. Reconcile before issuing receipts.' :
        ...
        status.fiscalDayStatus==='FiscalDayCloseFailed' ? 'Closure failed: '+(status.fiscalDayClosingErrorCode || 'inspect response')+'. Reconcile and retry closure.' : !open ? 'Open the production fiscal day to issue receipts.' :
        !dayAligned ? 'This open day was not initialized by this production service. Reconcile local state.' :
        !dayTimeValid ? 'Fiscal day timing is unavailable. Reconcile before issuing receipts.' :
        expired ? 'Fiscal day expired. Close the production day, refresh until closed, then open a new day before issuing another invoice.' : endingSoon ? 'Fiscal day ends in ' + remainingHours.toFixed(1) + ' hours. Close the day before expiry.' : 'Connected to Maxwell Glass production.' };
  }
  open() { return this.exclusive(async () => {
    ...
    const response = await this.request('/Device/v1/46158/OpenDay', { fiscalDayOpened: openedAt }, true);
  ...
  close() { return this.exclusive(async () => {
    ...
    this.state.closing.response = await this.request('/Device/v1/46158/CloseDay', payload, true); this.persist();
    return this.live();
  }); }
  submit(input) { return this.exclusive(async () => {
    ...
    if (!pending) {
      ...
      if (prepared.receipt.receiptType !== 'FiscalInvoice') {
        ...
        prepared.original = { receiptId: original.response.receiptID, invoiceNo: r.invoiceNo, globalNo: r.receiptGlobalNo, date: r.receiptDate, deviceId:46158, serial:'ZIMRAVD-1737' };
      }
    ...
    let response;
    try {
      response = pending.response || await this.request('/Device/v1/46158/SubmitReceipt', { receipt: pending.prepared.receipt }, true);
    } catch (error) {
  ...
  async maintenance() {
    if (this.busy) return;
    try {
      const ping = await this.request('/Device/v1/46158/Ping', {}, true);
  ...
  backup() { return this.exclusive(async()=>{
    const destination=path.resolve(transport.config.FDMS_BACKUP_DIR || path.resolve(this.directory,'../../backups'),'production-46158-'+new Date().toISOString().replace(/[:.]/g,'-'));
    ...
    if(restored.deviceId!==46158||audit.deviceId!==46158||audit.invoices.length!==restored.receipts.length)throw Error('Backup identity/count verification failed.');
