...
async function verifyFDMS(signature, message, request) {
  ...
  // Certificate is retrieved over authenticated HTTPS from the pinned sandbox host.
...
async function verifyDay(day, status, request) {
  ...
  const message='38293'+p.fiscalDayNo+day.openedAt.slice(0,10)+status.fiscalDayClosed+status.fiscalDayReconciliationMode.toUpperCase()+buildCounterString(status.fiscalDayCounter || p.fiscalDayCounters)+(status.fiscalDayReconciliationMode==='Manual'?'':p.fiscalDayDeviceSignature.signature);
...
async function renewCertificate(request) {
  ...
  execFileSync('C:/Program Files/Git/usr/bin/openssl.exe', ['req','-new','-sha256','-key',transport.keyPath,'-out',csr,'-subj','/C=ZW/O=Zimbabwe Revenue Authority/CN=ZIMRA-TEST-2000945150-B670-0000038293'], { windowsHide: true });
  const response = await request('/Device/v1/38293/IssueCertificate', {certificateRequest: fs.readFileSync(csr,'utf8')}, true);
